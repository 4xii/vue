<!--
 * @Author: 朱世新
 * @Date: 2021-06-09 23:24:13
 * @LastEditors: 朱世新
 * @LastEditTime: 2021-06-09 23:31:25
 * @Description: 
-->
<template>
  <!-- stripe:是否为斑马纹 table; -->
      ...
      <template slot-scope="scope">
        ...
        <div v-else>
          ...
          <span v-else>
            ...
            <el-tag v-if="th.isok == 1 && scope.row[th.prop] == true">{{
              scope.row[th.prop] | formatters(th.formatData)
            }}</el-tag>
            <el-tag
              v-else-if="th.isok == 1 && scope.row[th.prop] == false"
              type="success"
              >{{ scope.row[th.prop] | formatters(th.formatData) }}</el-tag
            >
            <!-- ** end**-->
            <span v-else>{{
              scope.row[th.prop] | formatters(th.formatData)
